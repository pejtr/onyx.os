...
export function getGitHubEvidencePolicy(): GitHubEvidencePolicy {
  return {
    token: ENV.githubReadToken,
    allowedRepos: parseRepoSet(ENV.githubAllowedRepos),
    trustedRepos: parseRepoSet(ENV.githubTrustedRepos),
  };
}
...
export function resolveGitHubRepoAccess(
  ...
  const allowed = policy.allowedRepos.has(repoKey);
  const useToken = Boolean(policy.token && allowed);
  const trust: KnowledgeTrust = policy.trustedRepos.has(repoKey)
    ? "trusted"
    : "unknown";
...
export async function fetchGitHubEvidenceFile(
  ...
  try {
    const response = await fetchImpl(
      buildContentsUrl(owner, repo, path, ref),
      {
        method: "GET",
        headers,
        redirect: "error",
    ...
    if (!response.ok) {
      throw new Error(
        `GitHub evidence read failed: HTTP ${response.status}`
      );
    }
    ...
    const chunks: KnowledgeChunk[] = pieces.map((piece, index) => ({
      id: `github:${access.repoKey}:${path}:${ref}:${index}`,
      text: piece.text,
      trust: access.trust,
      provenance: {
        sourceId: `github:${access.repoKey}`,
        sourceKind: "github",
        documentId: `${access.repoKey}:${path}`,
        chunkId: `${path}#${index + 1}`,
